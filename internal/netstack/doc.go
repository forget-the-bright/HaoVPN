...
//   ics_nat_windows.go — ICS 回退 SNAT；RememberICSPair；disable 优先 DisableICSPair
...
//   winnet_facade.go — ConfigureWindows/ShutdownWindows/HasICSResidue 等（供 clientapp，切断 clientapp→winnet）
//
// 上游：clientapp runtime、serverapp TUN/NAT Setup。
// 下游：winnet（Windows 网卡/PS/SKU）、netutil（CIDR）、platform（无窗口子进程）。
// 并发：Setup/Teardown 由调用方串行；路由变更非线程安全须单 goroutine 编排。
// 不变量：不 import tun；断线杀开关先于清路由（clientapp 编排）；
// PowerShell：NetNat/ICS 用 RunPSOneShot；清理用 BestEffort；禁止 raw platform.Command。
package netstack
