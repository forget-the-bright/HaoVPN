...
//     handler_dns_servers.go — 托管路由、互访、LAN 注册、托管 DNS（HTTP 薄层；写经 vpnaccount.PeerPolicyApplier）
//   handler_peers_apply.go / handler_peers_dirty.go — pending_apply 脏集与踢线刷新（只踢在线）
//   handler_vpn_peers_policy.go — 全局互访开关
//   handler_ops.go / monitor_handler.go（含 flows）/ handler_security_*.go / logs.go — 运维与探针
//
// 业务边界：
//   - 账号 IP 模式 / AllowedIPs / 启禁 / 删号 → vpnaccount（ApplyVPNPatch、SetAccountEnabled、DeleteAccount）
//   - 托管路由 / 互访 / 托管 DNS 写库+标脏 → vpnaccount.PeerPolicyApplier
//   - LAN 注册列表只读 → persist；应用生效 Kick+ListOnline 由 serverapp 注入 sessionmgr
//   - HTTP 读模型 DTO → readmodel
//
// 上游：serverapp 启动 Listen；WebUI / 脚本调用 REST。
// 下游：auth、vpnaccount、sessionmgr、persist、readmodel、paginate、timeutil、netutil、config、probedefense、flowmon。
